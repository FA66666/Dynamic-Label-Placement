...
def iterate(
    ...
    mid_x = (coords[:, 0] + coords[:, 1]) / 2
    mid_prev_x = (prev_coords[:, 0] + prev_coords[:, 1]) / 2
    mid_y = (coords[:, 2] + coords[:, 3]) / 2
    mid_prev_y = (prev_coords[:, 2] + prev_coords[:, 3]) / 2
    movement = np.sqrt((mid_x - mid_prev_x)**2 + (mid_y - mid_prev_y)**2)

    # 更新稳定计数器和活跃状态
    for i in range(len(coords)):
        if movement[i] > move_threshold:
            stable_counts[i] = 0
        else:
            stable_counts[i] += 1
        if stable_counts[i] >= stable_iterations:
    ...
    active_target = target_coords[active_indices]

    # 计算文本排斥力（仅对活跃点）
    text_shifts_x, text_shifts_y = get_shifts_texts(
        expand_coords(active_coords, expand[0], expand[1])
    )

    # 计算静态排斥力（静态点不受活跃状态影响）
    if static_coords is not None and static_coords.shape[0] > 0:
        static_shifts_x, static_shifts_y = get_shifts_extra(
            expand_coords(active_coords, expand[0], expand[1]),
            static_coords
        )
    else:
        static_shifts_x, static_shifts_y = np.zeros_like(text_shifts_x), np.zeros_like(text_shifts_y)
    # 计算总误差
    ...
    # 计算吸引力（仅对活跃文本）
    pull_x, pull_y = pull_back(active_coords, active_target)
    ...
    if only_move:
        ...
        if "y" not in only_move.get("pull", "xy"):
            ...
            # 合并偏移量（仅对活跃点）
    shifts_x_active = text_shifts_x + static_shifts_x + pull_x
    shifts_y_active = text_shifts_y + static_shifts_y + pull_y

        # 创建全量偏移数组（非活跃点偏移量为0）
    shifts_x = np.zeros(len(coords))
    shifts_y = np.zeros(len(coords))
    shifts_x[active_indices] = shifts_x_active
    shifts_y[active_indices] = shifts_y_active
    # 合并偏移量（仅对活跃文本）
    shifts_x_active = text_shifts_x + static_shifts_x + pull_x
    shifts_y_active = text_shifts_y + static_shifts_y + pull_y
    ...
    # 限制移动范围
    shifts_x = np.clip(
        np.sign(shifts_x) * np.ceil(np.abs(shifts_x)),
        -max_move[0], max_move[0]
    )
    shifts_y = np.clip(
        np.sign(shifts_y) * np.ceil(np.abs(shifts_y)),
        -max_move[1], max_move[1]
    )
    ...
    return coords, error, active_indices, stable_counts
...
def adjust_text(
    ...
    time_lim=None,
    iter_lim=None,
    move_threshold=0.1,  # 新增参数
    stable_iterations=5,  # 新增参数
    *args,
    **kwargs
):
    ...
    stable_counts = np.zeros(len(texts), dtype=int)
    active_indices = np.ones(len(texts), dtype=bool)
    ...
    while error > 0:
        # 调用迭代函数，传递状态参数
        coords, error, active_indices, stable_counts = iterate(
            coords,
