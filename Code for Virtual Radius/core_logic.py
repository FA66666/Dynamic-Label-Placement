...
class Label:
    ...
    def calculate_pos_at_frame(self, frame_idx, placer):
        ...
        if current_anchor is None:
            # 如果当前帧没有锚点观测值，则使用卡尔曼滤波的预测值
            last_observed_frame = max(self.feature.positions.keys()) if self.feature.positions else 0
            current_anchor = self.feature.predict_future(frame_idx - last_observed_frame)
            
        w, h = self.feature.size
        
        if self.avoidance_plan and self.avoidance_plan['start_frame'] <= frame_idx:
            # 如果有躲避计划，则使用线性插值更新 current_offset
            plan = self.avoidance_plan
            
            if frame_idx >= plan['end_frame']:
                # 计划已结束，直接设置为目标位置并清除计划
                self.current_offset = plan['target_offset']
                ...
                # 计算插值进度 (从 0.0 到 1.0)
                progress = (frame_idx - plan['start_frame']) / duration
                
                # 线性插值: new = start + (end - start) * progress
                delta = target_offset - start_offset
                self.current_offset = start_offset + delta * progress
