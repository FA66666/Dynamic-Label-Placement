...
class LabelOptimizer:
    ...
    def optimize(self):
        ...
        first_frame_positions = {}  # 使用统一的键格式存储
        ...
        for idx in range(len(self.labels)):
            ...
            if idx in first_frame_positions:
                combined_positions[idx] = first_frame_positions[idx]
            else:
                combined_positions[idx] = self.labels[idx].position

        # 最终返回的结果也使用统一的键格式
        full_set = {'set': list(range(len(self.labels)))}
        final_positions = self.simulated_annealing(combined_positions, full_set)
        
        # 转换为标签ID格式，供main.py使用
        final_id_positions = {self.labels[idx].id: pos for idx, pos in final_positions.items()}
