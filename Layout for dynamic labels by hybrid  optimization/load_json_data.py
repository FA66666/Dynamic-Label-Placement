            ...
            for i, point_id in enumerate(point_ids):
                if point_id in frame_points:
                    ...
                    if trajectories[i]:
                        ...
                        trajectories[i].append((100.0, 100.0))  # 默认位置
...
if __name__ == '__main__':
    # 测试加载功能
    trajectories = load_trajectories_from_json('sample_generated.json')
    if trajectories:
        print(f"轨迹数据加载完成，每个轨迹包含 {len(trajectories[0])} 个点")
        
        # 显示前几个点的轨迹信息
        for i, trajectory in enumerate(trajectories[:3]):
            print(f"点 {i}: 起始位置 {trajectory[0]}, 结束位置 {trajectory[-1]}")
    
    # 测试标签信息获取
    label_info = get_label_info_from_json('sample_generated.json')
    if label_info:
        print(f"标签信息获取完成，共 {len(label_info)} 个标签")
        for info in label_info[:3]:
            print(f"标签 {info['id']}: 文本='{info['text']}', 尺寸=({info['length']}, {info['width']})")
